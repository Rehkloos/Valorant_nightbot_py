...
def GetStats(name: str = "", tag: str = "", type: str = "") -> Player:
    ...
    # Initialise Player
    player = Player()
    player.damage.kda = float(
        results.find_all("span", class_="valorant-highlighted-stat__value")[-1].text
    )  # Get KDA
    ...
    for i in range(len(weapons)):
        ...
        player.weapons[i].kills = int(
            weapon.find("span", class_="value").text.replace(",", "")
        )
        pass

    if type == "competitive":
        rank_stats = results.find_all(
            "span", class_="valorant-highlighted-stat__value"
        )[0].text
        ...
        segments = {"status": status, "segements": api}
...
def rank_check(Region: str = "", Name: str = "", Tag: str = ""):
    ...
    rank_number = apiResponse["data"]["currenttier"]
    elo = apiResponse["data"]["elo"]
    status = apiResponse["status"]
    rank = res.ranks[str(rank_number)]
    full_rank_info = "Rank: " + rank + " | Elo: " + str(elo)

    if status == "200":
        return str(full_rank_info)
    elif status == "451":
...
def v1(name, tag, type):
    return current_app.response_class(
        json.dumps(GetStats(name, tag, type), indent=4), mimetype="application/json"
    )
